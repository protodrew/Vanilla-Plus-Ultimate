...
vec3 fxaa311(vec3 color){
	...
	float lumaDown = luma(texture2D(colortex0,texcoord.xy+vec2(0.0f,-1.0f)*view).rgb);
	float lumaUp = luma(texture2D(colortex0,texcoord.xy+vec2(0.0f,1.0f)*view).rgb);
	float lumaLeft = luma(texture2D(colortex0,texcoord.xy+vec2(-1.0f,0.0f)*view).rgb);
	float lumaRight = luma(texture2D(colortex0,texcoord.xy+vec2(1.0f,0.0f)*view).rgb);
	...
	if (lumaRange > max(edgeThresholdMin, lumaMax * edgeThresholdMax)){
		...
		float lumaDownLeft = luma(texture2D(colortex0,texcoord.xy+vec2(-1.0f,-1.0f)*view).rgb);
		float lumaUpRight = luma(texture2D(colortex0,texcoord.xy+vec2(1.0f,1.0f)*view).rgb);
		float lumaUpLeft = luma(texture2D(colortex0,texcoord.xy+vec2(-1.0f,1.0f)*view).rgb);
		float lumaDownRight = luma(texture2D(colortex0,texcoord.xy+vec2(1.0f,-1.0f)*view).rgb);
		...
		float lumaEnd1 = luma(texture2D(colortex0,uv1).rgb);
		float lumaEnd2 = luma(texture2D(colortex0,uv2).rgb);
		...
		if (!reachedBoth){
			for(int i = 2; i < iterations; i++){
				...
				if (!reached1){
					lumaEnd1 = luma(texture2D(colortex0, uv1).rgb);
					lumaEnd1 = lumaEnd1 - lumaLocalAverage;
				}
				// If needed, read luma in opposite direction, compute delta.
				if (!reached2){
					lumaEnd2 = luma(texture2D(colortex0, uv2).rgb);
		...
		aa = texture2D(colortex0,finalUv).rgb;
	}
	return aa;
}

void main(){
	vec3 color = fxaa311(texture2D(colortex0,texcoord.xy).rgb);
